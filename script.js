function buildFisheyeMap(size, strength) {
  ...
  for (let py = 0; py < size; py++) {
    for (let px = 0; px < size; px++) {
      ...
      const srcR = r > 0 ? r / (1 + strength * r * r) : 0;
      const f = r > 0 ? srcR / r : 1;
      const dx = nx * f - nx;
      const dy = ny * f - ny;
...
document.getElementById('fisheye-map').setAttribute('href', buildFisheyeMap(256, 0.5));
...
function loadVideo(filename) {
  ...
  video.onerror = () => {
    video.onerror = null;
    video.src = 'RANDOMACCESS/' + filename;
    video.play().catch(() => {});
  };
  video.play().catch(() => {});
...
document.addEventListener('click', () => {
  if (!started) {
    ...
    const current = video.src.split('/').pop();
    loadVideo(pickRandom(current));
  }
});
